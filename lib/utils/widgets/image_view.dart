...
enum ImageType { asset, file, network, svg, networkSvg, base64 }
...
class ImageView extends StatelessWidget {
  ...
  Widget get _base64 => image.isNotEmpty
      ? Image.memory(
          base64Decode(image),
          alignment: alignment,
          fit: boxFit,
  ...
  // TODO: change hardcoded colors with themeOf().cardBorderColor

  Widget get _loaderBuilder => Container(
        width: width,
        height: height,
        color: /*themeOf().cardBorderColor*/ const Color(0x2E08263A),
        child: Center(
          child: _loader,
        ),
      );
...
String getBase64Image(String? image) {
  if (image != null && image.isNotEmpty == true) {
    if (image.contains("https") == true) {
      return image;
    } else {
      String? convertedImage = image.replaceAll('-', '+');
      return convertedImage.replaceAll('_', '/');
    }
  } else {
    return "";
  }
